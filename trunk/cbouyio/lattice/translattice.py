...
class RandomObject(object) :
  """Superclass from which all the random number generators' classes should
  subclassing from.
...
class SignalTimestep(Parameter) :
  ...
  def __init__(self, a, b) :
    ...
    if isinstance(b, int) and isinstance(a, (int, float,)) :
      ...
      raise TypeError, "Check the types of the signal timestep parameters. Signal should either be 'int' or 'float' and timestep always 'float'"



class ControlExperimentParameter(Parameter) :
  """Class to represent the control experiment.

  Can be either a well stirred reactor (wsr) experiment or an individual
  collection of cells (icc) experiment.

  @ivar name: The name of the control experiment.
  @type name: C{str}
  """
  ...
  def __init__(self, experimentName) :
    ...
    @param experimentName: The name of the control experiment.
    @type experimentName: C{str}
    """
    if experimentName not in ('wsr', 'icc',) :
      raise StandardError, 'COntrol experiment name error, control experiment name can not recognised.'
    else :
...
class SimulatorControlParameters(object) :
  ...
  @ivar controlExpName: Specifies the control experiment's name. Can either be
  'wsr' or 'icc', default 'None'.
  @type controlExpName: C{str}
  ...
  @todo: Implement a factor initialisation dictionary class and the relevant
  instance variable.
  """

  def __init__(self, latticeSize, timesteps, initialisationVariables = None, factorInitialisation = None, signalTimestep = None, controlExp = None, randomSeed = 1, samplingInterval = 1) :
    """The constructor of the class
    ...
    self.controlExp = controlExp
    #FIXME: Implement the factor initalisation class.
    self.factorInitialisation = factorInitialisation


  def __str__(self) :
    """Print a commented line of all the simulator control parameters values.
...
#    transsys.TranssysInstance.__init__(self, tp, timestep)
  ...
  def time_series(self, num_timesteps, sampling_period=1, lsys_lines=None, lsys_symbol=None) :
    ...
    """
    timeseries = transsys.TranssysInstance.time_series(self, num_timesteps, sampling_period)
...
class TranssysInstanceLattice(transsys.TranssysInstanceCollection) :
  ...
  def perturb_lattice(self, perturbObj) :
    ...
    for latticeRow in self.lattice :
      for ti in latticeRow :
        for k in xrange(len(ti.factor_concentration)) :
          ti.factor_concentration[k] = self.perturb_factor(ti.factor_concentration[k], perturbObj)
  ...
  def write_table_header(self, outfile, sCP) :
    ...
    @type outfile: C{file}
    @param sCP: The simulator's control parameter object.
    L{SImulatorControlParameter}
    @type sCP: C{class 'SimulatorControlParameter'}'
    @rtype: C{None}
    @precondition: An open, ready for writting file object.
    """
    outfile.write(str(sCP))
    outfile.write('# Table of coordinated (x, y) factor concentration levels (Header file)\n')
...
#          raise StandardError, 'Factor number inconsistancy between the new and the old factor concentrations list.'
  ...
  def stir_well(self, rndSeed) :
    ...
    @param rndSeed: The random number generator initialisation seed.
    @param rndSeed: C{int}
    @rtype: C{'None'}
    """

    if type(rndSeed) is not int :
      raise StandardError, 'Random number generator seed should be integer.'
  ...
  def timestep_factor_concentration(self, signalC) :
    ...
    for latticeRow in self.lattice :
      for ti in latticeRow :
        for k in xrange(len(ti.factor_concentration)) :
          ti.factor_concentration[k] = signalC
...
class TranssysLatticeTimeseries(object) :
  ...
  @ivar transsysLattice: An transsys lattice object.
  @type transsysLattice: C{class 'TranassysInstanceLattice'}
  @ivar rndSeed: The random number generator seed of the simulator.
  @type rndSeed: C{int}
  @ivar timesteps: The number of timesteps of the simulation.
  @type timesteps: C{int}
  @ivar samplingInterval: The interval between sampling.
  @type samplingInterval: C{int}
  @ivar timestepSignal: The timestep signal object for the timeseries.
  @type timestepSignal: C{class 'SignalTimestep'}
  @ivar controlExp: The type of the control experiment to be run.
  @type controlExp: C{class 'ControlExperimentParameter'}
  ...
  def __init__(self, transsysLattice, simulatorCP) :
    """Constructor of the class.
    ...
    @param simulatorCP: An object containing all the simulator's control
    parameters, it is an instance of the L{SimulatorControlParameters} class.
    @type simulatorCP: C{class 'SimulatorControlParameters'}
    """
    self.transsysLattice = transsysLattice
    self.scp = simulatorCP
    self.rndSeed = simulatorCP.randomSeed
    self.timesteps = simulatorCP.timesteps
    self.samplingInterval = simulatorCP.samplingInterval
    self.timestepSignal = simulatorCP.signalTimestep
    self.controlExp = simulatorCP.controlExp
    self.latticeTimeseries = self.transsys_lattice_timeseries()


  def transsys_lattice_timeseries(self) :
    """Keep all the L{TranssysInstanceLattice} instances in a list.

    Return a timeseries of L{TranssysInstanceLattice} objects for all the
    simulation procedure.

    @return: The whole timeseries data of the simulator. The number of elements
    depends on the sampling interval.
    @rtype: A C{list} of C{class 'TranssysInstanceLattice'} objects.
    """
    # First check fo the individual collection of cells (unstructured) control
    # experiment.
    if self.controlExp and self.controlExp.name == 'icc' :
      self.transsysLattice.transsysProgram = zero_diffusibility(self.transsysLattice.transsysProgram)
    latticeTimeseries = []
    # Iterate for timestep + 1 to include the initial (zero) timestep. 
    for i in xrange(self.timesteps + 1):
      self.transsysLattice.timestep = i
      # Put a timestep signal if it is specified.
      if self.timestepSignal :
        if i == self.timestepSignal.timestep :
          self.transsysLattice.timestep_factor_concentration(self.timestepSignal.signal)
      # Append the deepcopy according to the sampling intervals.
      if i % self.samplingInterval == 0 :
        latticeTimeseries.append(copy.deepcopy(self.transsysLattice))
      self.transsysLattice.update_function(i)
      # Stir well if it is specified.
      if self.controlExp and self.controlExp.name == 'wsr' :
        self.transsysLattice.stir_well(self.rndSeed)
  ...
  def write_factor_table(self, infile) :
    ...
    self.latticeTimeseries[0].write_table_header(infile, self.scp)
    for til in self.latticeTimeseries :
      til.write_table(infile)


  # This function was needed only for the old animation approach. Obsolete
  # after the complete R animation approach. Therefore the max_concentration
  # instance variable does not exist anymore.
...
def zero_diffusibility(tp) :
  """Set a transsys program factor's diffusibilities to zero.

  Required to run the individual control experiment. By setting all the factors
  diffusibility to zero effectivelly isolates the cells on the lattice from
  each other.
  @return: The zero diffusibilities transsys program.
  @rtype: C{class 'transsys.TranssysProgram'}
  """
  tpZero = copy.deepcopy(tp)
  for diffusion in tpZero.getDiffusibilityValueNodes() :
    diffusion.value = 0.0
  return tpZero
